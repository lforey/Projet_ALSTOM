...
def prepare_iforest_features(df, window_size=50, calib_points=1000):
    """Calculates features and applies local calibration (normalization) for BOTH Vp and Ip."""
    # Feature Engineering Vp
    trend_vp = df['Vp'].rolling(window_size).median()
    df['Vp_Residual'] = (df['Vp'] - trend_vp).abs() # Correction de la parenthèse pour le .abs()
    df['Vp_Std'] = df['Vp'].rolling(window_size).std()
    df['Vp_Speed'] = df['Vp'].diff().abs().fillna(0)
    
    # Feature Engineering Ip (Ajout)
...
def evaluate_and_plot(test_results, threshold, true_anomaly_time, test_id):
    """Calculates metrics and plots the results (Now with 3 subplots to show Ip)."""
    ...
    # Plotting - Modified to 3 subplots
    ...
    # AI Score Plot
    ax3.plot(test_results['time'], test_results['Smoothed_Score'], color='purple', linewidth=1, label="AI Score")
