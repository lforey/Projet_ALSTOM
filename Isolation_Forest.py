...
def prepare_iforest_features(df, window_size=50, calib_points=1000):
    """Calculates features and applies local calibration (normalization)."""
    # Feature Engineering
    ...
    features_to_scale = ['Residual', 'Vp_Std', 'Speed']
    ...
    df_feat['Speed_scaled'] = scaled_features[:, 2]
...
def evaluate_and_plot(test_results, threshold, true_anomaly_time, test_id):
    ...
    # Changed 'Time' to 'time' here
    ax2.plot(test_results['time'], test_results['Smoothed_Score'], color='purple', linewidth=1, label="AI Score")
