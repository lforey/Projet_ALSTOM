...
def evaluate_and_plot_xgb(test_results, threshold, true_anomaly_time, pre_arc_target, test_id):
    alarm_indices = test_results.index[test_results['Smoothed_Proba'] > threshold].tolist()
    safe_zone_end_time = true_anomaly_time - pre_arc_target
    ...
    ax1.axvspan(true_anomaly_time - pre_arc_target, true_anomaly_time, color='green', alpha=0.3, label="Target Pre-Arc Window")
    ax1.set_ylabel("Voltage (V)")
    ax1.legend(loc='upper left')
    ax1.set_title(f"XGBoost Supervised Prediction: {test_id}")

    ax2.plot(test_results['time'], test_results['Smoothed_Proba'], color='purple', linewidth=1.5, label="Arc Probability")
